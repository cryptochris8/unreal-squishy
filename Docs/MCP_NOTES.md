...
**⚠️⚠️ NEVER leave an Enhanced-Input mapping context that maps the SAME physical key as a legacy `InputKey` event (double-fire).** We kept the legacy LMB/F squish events BUT also still added `IMC_Squishy` (LMB+F → `IA_Squish`, no handler). Result: every real key press was processed twice (legacy event AND the Enhanced-Input pass) → two `OnSquishPressed` calls per click → friends popped one squish early. It was invisible to synthetic `ProcessMouseButtonDownEvent` injection (that only triggers the legacy path), so it only showed on real hardware input and survived many "verified" fixes. **Fix:** don't add the redundant IMC at all (we disconnected `BeginPlay → AddSquishContext`). Rule: pick ONE input system per action — if using legacy `InputKey` events, do NOT also map that key in an active mapping context. (Fixed 2026-06-22.)

**⚠️ Side effect of legacy key events — they fire on the PIE focus/"join the world" click.** Unlike Enhanced Input (which ignores the focus-click via `bIgnoreAllPressedKeysUntilRelease` on the mapping context), a legacy `InputKey` `Pressed` fires when you click to focus the PIE viewport — so that click triggers your gameplay action once for free. This caused friends to Happy-Pop on the 2nd deliberate click instead of the 3rd. **Fix pattern (arm-on-first-release):** add a bool `SquishArmed` (default false); wire the key's **`Released`** pin(s) → `Set SquishArmed = true`; gate the action behind `Branch(SquishArmed)`. The focus-click's Pressed is ignored, its Release arms, every real press after works. Build it **surgically** (`create_node`/`connect_pins`) — `write_graph_dsl` erases legacy/Enhanced input-event bodies.
...
- **⚠️ `write_graph_dsl` positional args bind to the TARGET (`self`) pin first**: for a member/target function, the first positional arg goes to the node's `self` pin, NOT its first named parameter. `(Transformation|GetDistanceTo _friend)` puts `_friend` on `self` and leaves `OtherActor` empty → `friend.GetDistanceTo(null) = 0`. (Real bug: every friend read distance 0 → all squished on every click.) **Fix/avoid:** wire such nodes by pins (`break_pins`/`connect_pins`) or pass params by **keyword** (`:OtherActor _friend`); never trust positional on target-functions. Re-reading the DSL won't reveal it — the reader prints the same positional form regardless of real wiring; verify with `get_node_infos`.
- **⚠️ `write_graph_dsl` drops positional `true` on function bool pins**: a positional boolean **`true`** literal passed to a `K2Node_CallFunction` bool pin (e.g. `SetActorHiddenInGame true`, `SetActorEnableCollision true`) is **silently ignored** — the pin stays at its default (`false`). `false` literals and **variable-setter** bools (`SetPopped true`) are fine. After writing such a node, **verify with `get_node_infos` and force the pin** via `set_pin_value(pin, "true")` (input pin index 2 = the bool, after exec + self). The read-back DSL reflects the real (wrong) state, so re-read to catch it.
